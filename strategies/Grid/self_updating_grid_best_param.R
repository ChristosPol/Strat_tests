...
for (i in 2:nrow(df1)){
  ...
  current <- unique(short_grid$current)
  counter <- counter+1
  if(counter >params$days_reset[k]){
    ...
    short_grid <- data.table(batch=sample(x = 1:10E5,1), grid = seq(params$start[k], -0.5, by=params$step[k])[1:10], init_price = current, status_enter="open", status_exit = "open")[, entries := init_price+init_price*grid][, exits:=init_price+init_price*params$exit[k]]
    short_grid[, trade_id := replicate(nrow(short_grid), paste(sample(all_chars, str_len, replace = TRUE), collapse = ""))]
