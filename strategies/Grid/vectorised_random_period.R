...
exit_points <- data.table(exit=c(0.001,0.008, seq(0.01,0.05 ,0.01)), flag=1)
...
params <- params[splits_reset==1 & exit == 0.05 & start == 0.005 & maxim ==0.06 & n_trades==20]
...
for (x in 1:length(dataframes)){
  ...
  for (h in 1:nrow(params)){
    ...
    for (s in 1:length(list_df)){
      ...
      # Update grid table
      grid[position =="long" & status_enter =="open", interval_enter:=temp[first_index_long, interval]]
      grid[position =="long" & status_enter =="open", bar_entered:=first_index_long]
      grid[position =="short" & status_enter =="open", interval_enter:=temp[first_index_short, interval]]
      ...
      grid[position =="long" & status_enter =="closed"& status_exit=="open", interval_exit:=temp[first_index_long, interval]]
      grid[position =="long" & status_enter =="closed"& status_exit=="open", bar_exited:=first_index_long]
      grid[position =="short" & status_enter =="closed"& status_exit=="open", interval_exit:=temp[first_index_short, interval]]
