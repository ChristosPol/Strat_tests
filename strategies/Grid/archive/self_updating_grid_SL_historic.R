...
splits_reset <- data.table(splits_reset=c(4),flag=1)
...
for (h in 1:nrow(params)){
  ...
  for (s in 1:length(list_df)){
    ...
    closed_grid[[s]] <- grid[status_enter =="closed"]
    open_grid[[s]] <- grid[status_enter =="closed" & status_exit=="open"]
